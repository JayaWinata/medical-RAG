...
if __name__ == '__main__':
    ...
    rag_baseline = RAGBaseline(retriever=baseline_retriever, llm=llm)
    rag_fusion = RAGFusion(retriever=fusion_retriever, llm=llm, top_n=config['TOP_K'])
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for item in tqdm(dataset):
            ...
            # Fusion execution
            try:
                res_f = rag_fusion.run(query_to_run)
                fusion_data = {
                    "answer": res_f['answer'],
                    "ids": res_f['ids'],
